...
function saveList() {
  ...
  if(json_data == null) {
    ...
    json_data.recipes.push(jsonList); // append current list of lists with the new list
  }
  console.log(json_data);


  localStorage.setItem("recipes", JSON.stringify(json_data));
...
function makeListJSON(title, items) {
  let jsonList = '{"title":\"' + title + '\", "ingredients":[';
  var i;
  for (i = 0; i < items.length; i++) {
    if (i+1 == items.length)
      jsonList += '{"ingredient":\"' + items[i] + '\"}]}';
    else
      jsonList += '{"ingredient":\"' + items[i] + '\"},';
  }
...
function getNewJSON(title, items) {
  let jsonList = '{"recipes":[{"title":\"' + title + '\", "ingredients":[';
  var i;
  for (i = 0; i < items.length; i++) {
    if (i+1 == items.length)
      jsonList += '{"ingredient":\"' + items[i] + '\"}]}]}';
